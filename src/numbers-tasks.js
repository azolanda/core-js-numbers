...
function roundToPowerOfTen(num, pow) {
  const tenPowRound = 10 ** pow;
  return Math.round(num / tenPowRound) * tenPowRound;
}
...
function isPowerOfTwo(num) {
  let number = num;
  let remains = 0;
  while (number > 2 && !Number.isNaN(number) && !String(number).includes('.')) {
    remains = number % 2;
...
function getSumOfNumbers(x1, x2, x3) {
  return (
    Math.round([x1, x2, x3].reduce((sum, num) => sum + num, 0) * 100) / 100
  );
}
